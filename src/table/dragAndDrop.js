...
// Maximum column headers that will get drag listeners.
// Above this threshold the field has too many unique values to be a useful
// pivot column dimension — attaching listeners to all of them blocks the
// main thread and causes the UI to freeze on "Finalizing…".
const MAX_DRAGGABLE_COLUMNS = 200;
...
export function setupColumnDragAndDropFixed(columnFieldName) {
  ...
  if (columnHeaders.length > MAX_DRAGGABLE_COLUMNS) {
    logger.warn(
      `Skipping column drag setup: ${columnHeaders.length} headers exceed the ${MAX_DRAGGABLE_COLUMNS}-column limit.`,
      'The column field has too many unique values to be a useful pivot dimension.'
    );
    logger.info(
