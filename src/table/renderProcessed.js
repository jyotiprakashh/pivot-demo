...
// ── Debounce guard ─────────────────────────────────────────────────────────
// During engine initialisation, subscribe + setPagination + setDataHandlingMode
// + explicit call all fire renderTable() back-to-back.  We coalesce them into
// a single requestAnimationFrame so only ONE actual DOM render happens.
let _renderRAF = 0;
...
// The real implementation (called at most once per frame)
function _renderTableCore() {
  // Remove any lingering raw-data-preview (shown after CSV import)
  const rawPreview = document.getElementById('raw-data-preview');
  if (rawPreview) rawPreview.remove();
