...
function formatTable(newConfig, data) {
  try {
    ...
    // When `data` is supplied (large CSV import) use it directly to avoid
    // the expensive updateDataSource() + re-read cycle that doubles memory.
    appState.pivotEngine = new PivotEngine({
      ...newConfig,
      groupConfig: defaultGroupConfig || undefined,
      data: data || appState.pivotEngine.getState().rawData,
    });
